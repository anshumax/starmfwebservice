
package dev.anshumax.bsestarmf.additionalservices;

import jakarta.xml.bind.annotation.XmlAccessType;
import jakarta.xml.bind.annotation.XmlAccessorType;
import jakarta.xml.bind.annotation.XmlElement;
import jakarta.xml.bind.annotation.XmlRootElement;
import jakarta.xml.bind.annotation.XmlType;


/**
 * <p>Java class for anonymous complex type.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType&gt;
 *   &lt;complexContent&gt;
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType"&gt;
 *       &lt;sequence&gt;
 *         &lt;element name="ChildOrderDetailsResult" type="{http://schemas.datacontract.org/2004/07/StarMFWebService}ChildOrderResponse" minOccurs="0"/&gt;
 *       &lt;/sequence&gt;
 *     &lt;/restriction&gt;
 *   &lt;/complexContent&gt;
 * &lt;/complexType&gt;
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "", propOrder = {
    "childOrderDetailsResult"
})
@XmlRootElement(name = "ChildOrderDetailsResponse", namespace = "http://www.bsestarmf.in/2016/01/")
public class ChildOrderDetailsResponse {

    @XmlElement(name = "ChildOrderDetailsResult", namespace = "http://www.bsestarmf.in/2016/01/", nillable = true)
    protected ChildOrderResponse childOrderDetailsResult;

    /**
     * Gets the value of the childOrderDetailsResult property.
     * 
     * @return
     *     possible object is
     *     {@link ChildOrderResponse }
     *     
     */
    public ChildOrderResponse getChildOrderDetailsResult() {
        return childOrderDetailsResult;
    }

    /**
     * Sets the value of the childOrderDetailsResult property.
     * 
     * @param value
     *     allowed object is
     *     {@link ChildOrderResponse }
     *     
     */
    public void setChildOrderDetailsResult(ChildOrderResponse value) {
        this.childOrderDetailsResult = value;
    }

}
